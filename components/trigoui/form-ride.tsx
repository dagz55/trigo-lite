...
interface FormProps {
  onSubmit: (pickup: string, destination: string, e: React.FormEvent) => void | Promise<void>
...
export const FormRide = ({
  ...
  const [isInstantExitMode, setIsInstantExitMode] = useState(false)
  const [defaultExitPoint, setDefaultExitPoint] = useState<string | null>(null)

  useEffect(() => {
    // Load default exit point from local storage on component mount
    const savedExitPoint = localStorage.getItem("defaultExitPoint")
    if (savedExitPoint) {
      setDefaultExitPoint(savedExitPoint)
    }
  }, [])
  ...
  // Update destination when selectedTODA changes or Instant Exit mode is toggled
  useEffect(() => {
    if (isInstantExitMode && defaultExitPoint) {
      setDestinationValue(defaultExitPoint)
    } else {
      setDestinationValue(selectedTODA ? `${selectedTODA.name} exit point (labasan)` : "exit point (labasan)")
    }
  }, [selectedTODA, isInstantExitMode, defaultExitPoint])
  ...
  const handleSaveExitPoint = () => {
    if (destinationValue && destinationValue !== "exit point (labasan)") {
      localStorage.setItem("defaultExitPoint", destinationValue)
      setDefaultExitPoint(destinationValue)
      alert("Default exit point saved!")
    } else {
      alert("Please enter a valid destination to save as exit point.")
    }
  }

  const handleToggleInstantExit = () => {
    setIsInstantExitMode((prev) => !prev)
    if (!isInstantExitMode && defaultExitPoint) {
      setDestinationValue(defaultExitPoint)
    } else if (!isInstantExitMode && !defaultExitPoint) {
      alert("No default exit point saved. Please save one first.")
      setIsInstantExitMode(false) // Keep it off if no default
    }
  }

  return (
    <form onSubmit={(e) => onSubmit(pickupValue, destinationValue, e)} className="flex flex-col gap-2 flex-1 p-4 justify-between honeycomb-bg">
      <div className="space-y-4">
        {/* Instant Exit and Save Exit Point Buttons */}
        <div className="flex justify-between items-center gap-2">
          <Button
            type="button"
            onClick={handleToggleInstantExit}
            variant={isInstantExitMode ? "default" : "outline"}
            className={`flex-1 ${isInstantExitMode ? "bg-trigo-600 text-white hover:bg-trigo-700" : "border-trigo-200 dark:border-trigo-800 text-trigo-600 dark:text-trigo-400 hover:bg-trigo-50 dark:hover:bg-trigo-900/20"}`}
          >
            <Sparkles className="w-4 h-4 mr-2" />
            {isInstantExitMode ? "Instant Exit ON" : "Instant Exit OFF"}
          </Button>
          <Button
            type="button"
            onClick={handleSaveExitPoint}
            variant="outline"
            className="border-trigo-200 dark:border-trigo-800 text-trigo-600 dark:text-trigo-400 hover:bg-trigo-50 dark:hover:bg-trigo-900/20"
            disabled={!destinationValue || destinationValue === "exit point (labasan)"}
          >
            Save Exit Point
          </Button>
        </div>
        ...
        <div className="space-y-2 relative">
          ...
          <Input
            ...
            disabled={isInstantExitMode} // Disable input when Instant Exit is on
          />
          <p className="text-xs text-zinc-500">
            {isInstantExitMode && defaultExitPoint
              ? `Instant Exit to: ${defaultExitPoint}`
              : "Default destination is the nearest terminal exit point (\"labasan\")"}
          </p>

          {/* Destination suggestions */}
          {showDestinationSuggestions && destinationValue.length > 1 && !isInstantExitMode && (
