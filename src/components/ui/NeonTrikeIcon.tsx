...
const NeonTrikeIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg width="512" height="512" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg" {...props}>
    <defs>
      <linearGradient id="neon-trike-icon-autoGradient" x1="0.5" y1="0" x2="0.5" y2="1" gradientUnits="objectBoundingBox">
        <stop offset="0%" stopColor="#46E8BD"/> {/* Top: Bluish Green / Cyan */}
        <stop offset="30%" stopColor="#30F0A0"/>
        <stop offset="60%" stopColor="#10FF70"/> {/* Mid Green */}
        <stop offset="100%" stopColor="#00FF00"/> {/* Bottom: Bright Green */}
      </linearGradient>

      <filter id="neon-trike-icon-neonGlow" x="-50%" y="-50%" width="200%" height="200%">
        <feGaussianBlur stdDeviation="2.8" result="coloredBlur"/>
      </filter>

      <g id="neon-trike-icon-auto-rickshaw-shapes" strokeLinecap="round" strokeLinejoin="round" fill="none">
          {/* Main Body Outline */}
    ...
    <g transform="translate(20.5, 5.5)">
      ...
      <use href="#neon-trike-icon-auto-rickshaw-shapes" stroke="url(#neon-trike-icon-autoGradient)" strokeWidth="6" filter="url(#neon-trike-icon-neonGlow)"/>
      {/* Main Lines Layer: Thinner stroke, sharp */}
      <use href="#neon-trike-icon-auto-rickshaw-shapes" stroke="url(#neon-trike-icon-autoGradient)" strokeWidth="2"/>
    </g>
  </svg>
);

export default NeonTrikeIcon;
