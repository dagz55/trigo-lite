...
const BottomNavBar = () => {
  ...
  const navItems = [
    {
      href: '/passenger?view=transport', // Ensure this always navigates to the transport view
      icon: Home,
      label: 'Home',
      isActive: pathname === '/passenger' && (new URLSearchParams(window.location.search).get('view') === 'transport' || !new URLSearchParams(window.location.search).get('view'))
    },
    {
      href: '/passenger?view=ride_in_progress',
      icon: Map,
      label: 'Ride',
      isActive: pathname === '/passenger' && new URLSearchParams(window.location.search).get('view') === 'ride_in_progress'
    },
    {
