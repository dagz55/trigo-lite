...
// Payment method icon data (returns icon props instead of JSX)
export const getPaymentIconData = (method: PaymentMethod | string) => {
  const methodId = typeof method === 'string' ? method : method.id;

  switch (methodId) {
    case 'gcash':
      return { type: 'image', src: '/GCash.png', alt: 'GCash' };
    case 'paymaya':
      return { type: 'image', src: '/maya-logo.png', alt: 'PayMaya' };
    case 'tricoin':
      return { type: 'tricoin', symbol: '₮' };
    default:
      return { type: 'default' };
  }
};

// Legacy function for backward compatibility - will be replaced in components
export const getPaymentIcon = getPaymentIconData;
