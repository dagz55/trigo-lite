             ...
             <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                    ...
                    <AlertTitle className="text-white font-semibold">
                        {(() => {
                            // Within this IIFE, rideState.status should retain its broader type before the switch
                            const status = rideState.status;
                            if (status === 'confirmingRide') return 'Confirm Ride';
                            if (status === 'searching') return 'Finding TriDer...';
                            if (status === 'triderAssigned') return `TriDer En Route`;
             ...
             <AlertDescription className="space-y-1.5 text-white/90 text-sm">
                ...
                {rideState.currentRideId &&
                  (rideState.status === 'searching' ||
                   rideState.status === 'triderAssigned' ||
                ...
                {rideState.status === 'searching' && (
                  <p>Searching for the nearest available trider for you...</p>
                )}
