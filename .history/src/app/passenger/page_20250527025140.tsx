...
export default function PassengerPage() {
  ...
  const [pickupToDropoffRouteColor, setPickupToDropoffRouteColor] = React.useState('var(--passenger-accent-color)');
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
        ...
        setPickupToDropoffRouteColor('var(--passenger-accent-color)'); // Use CSS variable here
    }
  }, []);
        ...
        <div className="relative z-10 flex flex-col h-full p-6 pt-4">
          <header className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2">
              ...
              {/* Chat Button */}
              <Button variant="ghost" size="icon" className="text-white hover:bg-white/20 border-2 border-white/40 hover:border-white/60 rounded-lg transition-all duration-200" title="Messages/Chat" onClick={handleChatClick}>
           ...
           >
             <div className="flex items-center justify-between mb-2">
                ...
                {(rideState.status === 'triderAssigned' || rideState.status === 'inProgress') && rideState.countdownSeconds !== null && (
                    <div
                        className={`text-2xl ${countdownPulseClass} countdown-text-color`}
             ...
             <AlertDescription className="space-y-1.5 text-white/90 text-sm">
                ...
                {(rideState.status === 'searching' || rideState.status === 'triderAssigned' || rideState.status === 'inProgress' || rideState.status === 'completed' || rideState.status === 'cancelled' || rideState.status === 'confirmingRide' ) && rideState.currentRideId &&
                ...
                {rideState.status === 'completed' && (
                    <p>Thank you for using TriGo! We hope you had a great ride.</p>
                )}
                {rideState.status === 'cancelled' && (
                    <p>Sorry, your ride has been cancelled. Please try again.</p>
                )}
             </AlertDescription>
             {rideState.status === 'confirmingRide' && (
                <Button className={`w-full mt-4 main-button-color text-white`} onClick={handlePickMeUpNow} disabled={!rideState.pickupLocation || !rideState.dropoffLocation || rideState.estimatedFare === null} title="Request TriGo Now">
                    Request TriGo Now
                </Button>
             )}
           </Alert>
        ...
        {rideUpdates.length > 0 && (rideState.status !== 'confirmingRide' && rideState.status !== 'selectingPickup' && rideState.status !== 'selectingDropoff') && (
         ...
         {(rideState.status === 'completed' || rideState.status === 'cancelled') && (
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-10 w-full max-w-xs px-4">
                <Button className={`w-full main-button-color text-white`} onClick={() => { resetRideState(); setIsReceiptDialogOpen(false); }} title="Book Another Ride">
                    Book Another Ride
                </Button>
            </div>
         )}
