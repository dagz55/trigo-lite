...
const TriGoPassengerLogoInHeader = () => (
  <svg width="36" height="36" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-1">
    <circle cx="16" cy="16" r="13" stroke="var(--passenger-accent-color)" strokeWidth="1.5"/>
    <path d="M9.5 20V14C9.5 12.8954 10.3954 12 11.5 12H17.5C18.6046 12 19.5 12.8954 19.5 14V20" stroke="var(--passenger-header-text)" strokeWidth="1.2" fill="none"/>
    <path d="M M9.5 12 Q16 8 22.5 12 L19.5 12" stroke="var(--passenger-header-text)" strokeWidth="1.2" fill="none" />
    <path d="M19.5 13H21.5C22.0523 13 22.5 13.4477 22.5 14V17C22.5 17.5523 22.0523 18 21.5 18H19.5" stroke="var(--passenger-header-text)" strokeWidth="1.2" fill="none"/>
    <circle cx="12" cy="20.5" r="1.5" stroke="var(--passenger-header-text)" strokeWidth="1" fill="none"/>
    <circle cx="17" cy="20.5" r="1.5" stroke="var(--passenger-header-text)" strokeWidth="1" fill="none"/>
  </svg>
);
...
export default function PassengerPage() {
  ...
  const countdownPulseClass = rideState.countdownSeconds !== null && rideState.countdownSeconds <= 10 ? 'font-bold animate-pulse countdown-pulse' : 'font-bold'; // Add CSS pulse class
        ...
        <div className="relative z-10 flex flex-col h-full p-6 pt-4">
          <header className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
              {/* TriGo Logo */}
              <div className="w-8 h-8 rounded-full landing-header-logo-circle flex items-center justify-center">
              ...
              {[{ label: "Ride", subLabel: "Before", icon: History, action: handleRideBefore },
               ...
               { label: "Ride", subLabel: "Later", icon: CalendarCheck2, action: handleRideLater }
                ...
                <div key={item.subLabel} className="enhanced-button-container">
                  {/* Button-specific dotted lights */}
                  {Array.from({ length: 8 }, (_, i) => (
                    <div
                      key={`dotted-${idx}-${i}`}
                      className="button-dotted-light"
                      style={{
                        top: `${20 + Math.random() * 60}%`,
                        left: `${20 + Math.random() * 60}%`,
                        animationDelay: `${Math.random() * 3}s, ${Math.random() * 1.5}s`
                      }}
                    />
                  ))}

                  {/* Circling angel lights around button */}
                  <div className="button-circle-light button-circle-light-1" style={{ animationDelay: `${idx * 0.5}s` }} />
                  <div className="button-circle-light button-circle-light-2" style={{ animationDelay: `${idx * 0.5 + 1}s` }} />
                  <div className="button-circle-light button-circle-light-3" style={{ animationDelay: `${idx * 0.5 + 2}s` }} />
