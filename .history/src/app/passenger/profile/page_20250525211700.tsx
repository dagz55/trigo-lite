...
export default function PassengerProfilePage() {
  ...
  const handleTestPayment = React.useCallback(async (paymentMethod: PaymentMethod) => {
    ...
    // Test payment amount (₱25 for demo)
    const testAmount = 25;
              ...
              <>
                ...
                <div className="text-left">
                  <div className="font-semibold text-gray-900 flex items-center gap-2">
                    {selectedMethod.name}
                    {paymentState.status === 'success' && paymentState.transaction?.paymentMethodId === selectedMethod.id && (
                      <span className="text-green-600 text-xs">✓ Payment Success</span>
                    )}
                    {paymentState.status === 'error' && paymentState.transaction?.paymentMethodId === selectedMethod.id && (
                      <span className="text-red-600 text-xs">✗ Payment Failed</span>
                    )}
                  </div>
                  <div className="text-sm text-gray-500">
                    Balance: {formatCurrency(selectedMethod.balance || 0)}
                  </div>
                </div>
              </>
            ) : (
            ...
            <div className="py-2">
              {paymentMethods.map((method, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div
                      className="w-8 h-8 flex items-center justify-center cursor-pointer hover:scale-110 transition-transform duration-200 relative"
                      onClick={() => handleTestPayment(method)}
                      title={`Test ${method.name} payment (₱25)`}
                    >
